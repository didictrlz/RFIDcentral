...
class Reader(LLRPClient):
	def __init__(self, ip=dirección_ip, includeEPCs=[], excludeEPCs=[], *args, **kwargs):
...
class R420(Reader):
	...
	def detectTags(self, powerDBm=31.5, freqMHz=866.9, mode=1001, searchmode=0, **kwargs):
		'''
	...
	def startLiveReports(self, reportCallback, powerDBm=31.5, freqMHz=866.9, mode=1000, searchmode=0, **kwargs):
		self.impinj_searchmode = searchmode # update searchmode
...
class FX9600(Reader):
	...
	def startLiveReports(self, reportCallback, powerDBm=29.2, freqMHz=866.3, mode=2, session=0, **kwargs):
